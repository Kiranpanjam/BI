...
# A. Import data. Load character variable as character strings first (stringsAsFactors = FALSE). (2 points)
BartRider <- read.csv(file = "BartRider.csv", stringsAsFactors = FALSE)

# B.	Show the overall structure and summary of the input data. (2 points)
str(BartRider)
summary(BartRider)

# C.	Transform DualInc, Gender, Language, OwnRent, and Rider to factor variables. Show the overall structure and summary of the input data again. (2 points)
...
# D.	Show the number of rows as well as the number of columns of input data. (2 points)
nrow(BartRider)
ncol(BartRider)

# E.	Show the first 10 instances of input data. Also, show the last 10 instances of input data. (2 points)
head(BartRider, n = 10)
tail(BartRider, n = 10)

### 2. Code for exploring a single variable (10 points)
# A.	For each of the following variables - Age, and Education draw variable's boxplot. (2 points)
...
# B.	For each of the following variables - Income, and YrsInArea draw variable's histogram. (2 points)
...
# C.	For each of the following variables - DistToWork and NbrInHouseHold, show variable quantiles. (2 points)
quantile(BartRider$DistToWork)
quantile(BartRider$NbrInHouseHold)

# D.  Choose two factor variables. For each factor variable, show the number of instances (count) in each category/level. (2 points)
table(BartRider$Gender)
table(BartRider$Rider)

# E.  Choose two factor variables to plot. (2 points)
plot(BartRider$Gender, main="Plot of Gender in BartRider")
plot(BartRider$Rider, main="Plot of Rider in BartRider")

### 3. Code to understand relationships amongst multiple variables (10 points)
# A.  Display correlations for DistToWork and NbrInHouseHold. (2.5 points)
cor(BartRider$DistToWork, BartRider$NbrInHouseHold)

# B.  Display correlations for all numeric variables. (2.5 points)
cor(BartRider[,c(1,2,4,6,8,9,11)])

# C.  Show a boxplot of Income by Rider. (2.5 points)
boxplot(Income~Rider, data = BartRider)

# D.  Show a boxplot of Income by Rider for only Female. (2.5 points)
boxplot(Income~Rider, data = BartRider[which(BartRider$Gender == 'F'),])

### 4. Data partitioning and inspection code (10 points)
# A.	Partition the data set for simple hold-out evaluation - 50% for training and the other 50% for testing. (5 points)
library(caret)
set.seed(100)
...
# B.	Show the overall structure and summary of train and test sets. Show the distributions of Rider in the entire set, the train set and the test set. (5 points)
str(datTrain)
summary(datTrain)
str(datTest)
summary(datTest)
...
# A.	Train a C5.0 model using the default setting. (4 points)
...
# B.	Explore reducing the tree complexity by the lowering CF levels. (4 points)
...
# Which model has better performance (model in task A or B), and why? (2 points)
